export default async function GET(req: Request) {
    ...
    if (lyrics[0]) {
        ...
        if (splitLyric(timed)[0]) {
            ...
            return new Response(JSON.stringify({ song: track, lyrics: extract, synced: true }))
        } else {
            return new Response(JSON.stringify({ song: track,lyrics: lyrics[0].plainLyrics, synced: false }))
        }
    } else {
        ...
        if (l[0]) {
            ...
            if (timed) {
                ...
                return new Response(JSON.stringify({ song: track,lyrics: l[0].plainLyrics, synced: false }))
            }
        } else {
            return new Response(JSON.stringify({ song: track,error: 'No lyrics found' }), { status: 404 });
        }
    }
}
